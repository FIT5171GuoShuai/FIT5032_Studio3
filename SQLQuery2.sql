...
-- --------------------------------------------------
-- Date Created: 08/05/2021 12:22:14
...
-- Creating foreign key on [AdminId] in table 'HeritageSet'
ALTER TABLE [dbo].[HeritageSet]
ADD CONSTRAINT [FK_AdminHeritage]
    FOREIGN KEY ([AdminId])
    REFERENCES [dbo].[AdminSet]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_AdminHeritage'
CREATE INDEX [IX_FK_AdminHeritage]
ON [dbo].[HeritageSet]
    ([AdminId]);
GO

-- Creating foreign key on [AdminId] in table 'UserSet'
ALTER TABLE [dbo].[UserSet]
